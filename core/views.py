...
def table(request):
	...
	if query:
		tables = Table.objects.filter(Q(name__icontains=query) | Q(threat__icontains=query) | Q(source_of_threat__icontains=query) | Q(mechanism__icontains=query))
...
def integrity (request):
    ...
    query = request.GET.get('q')
    if query:
        tables = Table.objects.filter(table_category=TableCategory.objects.get(name='Цілісність')).filter(
        Q(name__icontains=query) | Q(threat__icontains=query) | Q(source_of_threat__icontains=query) | Q(mechanism__icontains=query))
    else:
        tables = sorted_tables
...
def accessibility (request):
    ...
    query = request.GET.get('q')
    if query:
        tables = Table.objects.filter(table_category=TableCategory.objects.get(name='Доступність')).filter(
        Q(name__icontains=query) | Q(threat__icontains=query) | Q(source_of_threat__icontains=query) | Q(mechanism__icontains=query))
    else:
        tables = sorted_tables
...
def privacy (request):
    ...
    query = request.GET.get('q')
    if query:
        tables = Table.objects.filter(table_category=TableCategory.objects.get(name='Конфіденційність')).filter(
        Q(name__icontains=query) | Q(threat__icontains=query) | Q(source_of_threat__icontains=query) | Q(mechanism__icontains=query))
    else:
        tables = sorted_tables
